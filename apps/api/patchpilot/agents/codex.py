...
class CodexCodingAgent:
    ...
    def _analysis_prompt(context: AgentTaskContext) -> str:
        return f"""Analyze GitHub issue #{context.issue_number}: {context.title}\n\n{context.description or ''}\nRepository: {context.repository}\nCoding guidelines: {context.coding_guidelines or 'none'}\nProtected paths: {context.protected_paths}\nTest command: {context.test_command or 'none'}\nLint command: {context.lint_command or 'none'}\nInspect only this workspace. Do not edit files. Return one JSON object matching the supplied schema. Use status decision_required for ambiguity or risky choices and include options. Store no chain-of-thought; provide concise observable summaries only."""

    @staticmethod
    def _implementation_prompt(context: AgentTaskContext) -> str:
        return f"""Implement issue #{context.issue_number}: {context.title}. Work only inside this repository. Do not modify protected paths: {context.protected_paths}. Do not push, merge, or access unrelated files. Add or update tests as appropriate. PatchPilot will run authoritative validation and policy checks. Return one JSON object matching the supplied schema with changed_files and observable actions only."""
