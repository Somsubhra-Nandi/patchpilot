...
export default function TaskDetailPage() {
  ...
  const resolveAgentDecision = useMutation({ mutationFn: (payload: { id: string; option: string }) => api.resolveDecision(payload.id, payload.option), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["task", taskId] }); queryClient.invalidateQueries({ queryKey: ["decisions"] }); } });
  const taskAction = useMutation({ mutationFn: (action: "pause" | "resume") => api.taskAction(taskId, action), onSuccess: (updated) => queryClient.setQueryData(["task", taskId], updated) });
  useEffect(() => {
  ...
  const pendingDecision = task.decisions?.find((item) => item.status === "pending");
  return (
    <div className="page">
      ...
      <div className="task-grid">
        ...
        <aside className="task-stack">
          {pendingDecision && <section className="panel"><div className="panel-header"><h2>Human decision required</h2><span className="badge badge-red">{pendingDecision.risk_level} risk</span></div><div className="content-section"><div className="approval-box"><strong>{pendingDecision.title}</strong><p>PatchPilot paused because the {pendingDecision.requested_by_agent} agent could not safely choose. {pendingDecision.context.relevant_files?.length ? `Relevant files: ${pendingDecision.context.relevant_files.join(", ")}.` : "Review the observable context before continuing."}</p><p><b>Recommendation:</b> {pendingDecision.recommended_option || "None"}</p><div className="approval-actions">{pendingDecision.options.map((option) => <button className={option.id === pendingDecision.recommended_option ? "button button-primary" : "button button-secondary"} disabled={resolveAgentDecision.isPending} onClick={() => resolveAgentDecision.mutate({ id: pendingDecision.id, option: option.id })} key={option.id}>{option.id}: {option.label}</button>)}</div></div></div></section>}
          {task.decisions?.some((item) => item.status !== "pending") && <section className="panel"><div className="panel-header"><h2>Decision history</h2></div><div className="content-section"><ul className="detail-list">{task.decisions.filter((item) => item.status !== "pending").map((item) => <li key={item.id}>{item.title}: {item.resolution} by {item.resolved_by} via {item.resolved_channel}</li>)}</ul></div></section>}
